    ...
    for month in range(10, 1, -1):
        ...
        for date in range(28 , 1, -1):
            ...
            for page in range(1, int(number)):
                ...
                url ='https://s.weibo.com/weibo?q=早餐&scope=ori&haspic=1&timescope=custom:'+str(year)+'-'+str(month)+'-'+str(date)+'-0:'+str(year)+'-'+str(month)+'-'+str(int(date)+1)+'-0&Refer=g&sudaref=s.weibo.com&display=0&retcode=6102&page='+str(page)
                ...
                t1 = threading.Thread(target=writetext,args=(text_link,))
                t2 = threading.Thread(target=writetime,args=(time_link,))
